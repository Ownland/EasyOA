...
public class CalendarActivity extends Activity {
	...
	private CreateHandler createHanlder = new CreateHandler();
	...
	public void initCalendarListView() {
		calendarTodoAdapter = new ListViewCalendarTodoAdapter(this,
				listTodoCalendarList, R.layout.calendars_listitem_todo);
		calendarTodoFooter = getLayoutInflater().inflate(
				R.layout.listview_footer_calendar_todo, null);
		calendarTodoFootMore = (TextView) calendarTodoFooter
				.findViewById(R.id.listview_foot_more_calendar_todo);
		calendarTodoFootProgress = (ProgressBar) calendarTodoFooter
				.findViewById(R.id.listview_foot_progress_calendar_todo);
		...
		calendarTodoList
				.setOnItemClickListener(new CalendarTodoItemClickListener());
		calendarTodoList
				.setOnScrollListener(new CalendarTodoListScrollListener());
		calendarTodoList
				.setOnRefreshListener(new CalendarTodoListRefreshListener());

		calendarDoneAdapter = new ListViewCalendarDoneAdapter(this,
				listDoneCalendarList, R.layout.calendars_listitem_done);
		calendarDoneFooter = getLayoutInflater().inflate(
				R.layout.listview_footer_calendar_done, null);
		calendarDoneFootMore = (TextView) calendarDoneFooter
				.findViewById(R.id.listview_foot_more_calendar_done);
		calendarDoneFootProgress = (ProgressBar) calendarDoneFooter
				.findViewById(R.id.listview_foot_progress_calendar_done);
		...
		calendarDoneList
				.setOnItemClickListener(new CalendarDoneItemClickListener());
		calendarDoneList
				.setOnScrollListener(new CalendarDoneListScrollListener());
		calendarDoneList
				.setOnRefreshListener(new CalendarDoneListRefreshListener());
	}

	public void initCalendarListViewData() {
		calendarTodoHandler = this.getCalendarTodoHandler(calendarTodoList,
				calendarTodoAdapter, calendarTodoFootMore,
				calendarTodoFootProgress, MyApplication.PAGE_SIZE);
		if (listTodoCalendarList.isEmpty()) {
			loadCalendarData(0, calendarTodoHandler,
					UIHelper.LISTVIEW_ACTION_INIT, true, false);
		}
		calendarDoneHandler = this.getCalendarDoneHandler(calendarDoneList,
				calendarDoneAdapter, calendarDoneFootMore,
				calendarDoneFootProgress, MyApplication.PAGE_SIZE);
		if (listDoneCalendarList.isEmpty()) {
			loadCalendarData(0, calendarDoneHandler,
					UIHelper.LISTVIEW_ACTION_INIT, false, false);
		}
	}

	private void loadCalendarData(final int pageIndex, final Handler handler,
			final int action, final boolean todo, final boolean isRefresh) {
		new Thread() {
			public void run() {
	...
	private void handleCalendarTodoData(int what, Object obj, int objtype,
			int actiontype) {
		switch (actiontype) {
			...
			if (actiontype == UIHelper.LISTVIEW_ACTION_REFRESH) {
				if (!new NetUtil(this).goodNet()) {
					...
					Toast.makeText(getApplicationContext(),
							R.string.updatre_complete, Toast.LENGTH_SHORT)
							.show();
				}
			}
			break;
				...
				if (listTodoCalendarList.size() > 0) {
					for (Calendarevent calendar1 : list.getCalendarslist()) {
						...
						for (Calendarevent calendar2 : listTodoCalendarList) {
							if (calendar1.getEventId() == calendar2
									.getEventId()) {
								b = true;
								break;
							}
						...
						if (!b)
							listTodoCalendarList.add(calendar1);
					}
				} else {
	...
	private void handleCalendarDoneData(int what, Object obj, int objtype,
			int actiontype) {
		switch (actiontype) {
		case UIHelper.LISTVIEW_ACTION_INIT:
		case UIHelper.LISTVIEW_ACTION_REFRESH:
			...
			case UIHelper.LISTVIEW_DATATYPE_EMAIL:
				...
				if (listDoneCalendarList.size() > 0) {
					for (Calendarevent calendar1 : list.getCalendarslist()) {
						...
						if (!b)
							listDoneCalendarList.add(calendar1);
					}
				} else {
	...
	private Handler getCalendarTodoHandler(final PullToRefreshListView lv,
			final BaseAdapter adapter, final TextView more,
			final ProgressBar progress, final int pageSize) {
		return new Handler() {
			public void handleMessage(Message msg) {
				if (msg.what >= 0) {
					handleCalendarTodoData(msg.what, msg.obj, msg.arg2,
							msg.arg1);
	...
	private Handler getCalendarDoneHandler(final PullToRefreshListView lv,
			final BaseAdapter adapter, final TextView more,
			final ProgressBar progress, final int pageSize) {
		return new Handler() {
			public void handleMessage(Message msg) {
				if (msg.what >= 0) {
					handleCalendarDoneData(msg.what, msg.obj, msg.arg2,
							msg.arg1);
					if (msg.what < pageSize) {
						lv.setTag(UIHelper.LISTVIEW_DATA_FULL);
						adapter.notifyDataSetChanged();
		...
		public void onItemClick(AdapterView<?> parent, View view, int position,
			...
			if (view instanceof TextView) {
				...
				TextView tv = (TextView) view
						.findViewById(R.id.calendar_title_todo);
			...
			intent.putExtra(Calendarevent.EVENT_ID, event.getEventId());
			intent.putExtra(Calendarevent.BEGIN_TIME, event.getBeginTime());
			intent.putExtra(Calendarevent.END_TIME,event.getEndTime());
			intent.putExtra(Calendarevent.NAME, event.getName());
			intent.putExtra(Calendarevent.LOCATION, event.getLocation());
			intent.putExtra(Calendarevent.DESCRIPTION,event.getDescription());
			intent.putExtra(Calendarevent.REMIND, event.isRemind());
			intent.putExtra(Calendarevent.OWNER_ID, event.getOwnerId());
			intent.putExtra(Calendarevent.VERSION,event.getVersion());
	...
	private class CalendarTodoListScrollListener implements OnScrollListener {
		...
		public void onScroll(AbsListView view, int firstVisibleItem,
			...
			// TODO Auto-generated method stub
			calendarTodoList.onScroll(view, firstVisibleItem, visibleItemCount,
					totalItemCount);
		}
		...
		public void onScrollStateChanged(AbsListView view, int scrollState) {
			...
			try {
				if (view.getPositionForView(calendarTodoFooter) == view
			...
			if (scrollEnd && lvDataState == UIHelper.LISTVIEW_DATA_MORE) {
				...
				loadCalendarData(pageIndex, calendarTodoHandler,
						UIHelper.LISTVIEW_ACTION_SCROLL, true, true);
			}
		}
	...
	private class CalendarTodoListRefreshListener implements OnRefreshListener {
		...
		public void onRefresh() {
			// TODO Auto-generated method stub
			loadCalendarData(0, calendarTodoHandler,
					UIHelper.LISTVIEW_ACTION_REFRESH, true, true);
	...
	private class CalendarDoneItemClickListener implements OnItemClickListener {
		...
		public void onItemClick(AdapterView<?> parent, View view, int position,
			...
			if (view instanceof TextView) {
				...
				TextView tv = (TextView) view
						.findViewById(R.id.calendar_title_done);
				event = (Calendarevent) tv.getTag();
			}
			if (event == null)
				return;
			...
			intent.putExtra(Calendarevent.LOCATION, event.getLocation());
			intent.putExtra(Calendarevent.DESCRIPTION,event.getDescription());
			intent.putExtra(Calendarevent.OWNER_ID,event.getEventId());
			intent.putExtra(Calendarevent.REMIND, event.isRemind());
			intent.putExtra(Calendarevent.OWNER_ID, event.getOwnerId());
			startActivity(intent);
		}
	...
	private class CalendarDoneListScrollListener implements OnScrollListener {
		...
		public void onScroll(AbsListView view, int firstVisibleItem,
				int visibleItemCount, int totalItemCount) {
			// TODO Auto-generated method stub
			calendarDoneList.onScroll(view, firstVisibleItem, visibleItemCount,
					totalItemCount);
		}
		...
		public void onScrollStateChanged(AbsListView view, int scrollState) {
			...
			boolean scrollEnd = false;
			try {
				if (view.getPositionForView(calendarDoneFooter) == view
						.getLastVisiblePosition())
					scrollEnd = true;
			} catch (Exception e) {
				scrollEnd = false;
			...
			if (scrollEnd && lvDataState == UIHelper.LISTVIEW_DATA_MORE) {
				...
				Log.e("pageIndex", pageIndex + "");
				loadCalendarData(pageIndex, calendarDoneHandler,
						UIHelper.LISTVIEW_ACTION_SCROLL, true, true);
			}
		}
	...
	private class CalendarDoneListRefreshListener implements OnRefreshListener {
		...
		public void onRefresh() {
			// TODO Auto-generated method stub
			loadCalendarData(0, calendarDoneHandler,
					UIHelper.LISTVIEW_ACTION_REFRESH, false, true);
		}
	...
	private class CreateHandler extends Handler {
		@Override
		public void handleMessage(Message msg) {
			Intent intent = new Intent(CalendarActivity.this,
					CalendarActivity.class);
